...
ADIS16470_IMU::ADIS16470_IMU(IMUAxis yaw_axis, SPI::Port port, ADIS16470CalibrationTime cal_time) : 
  ...
  if (!m_sim_device) {
    ...
    FRC_ReportError(frc::warn::Warning, "{}",
        "ADIS16470 IMU Successfully Initialized!");
...
bool ADIS16470_IMU::SwitchToAutoSPI(){
  ...
  if(m_spi == nullptr){
    if(!SwitchToStandardSPI()){
      FRC_ReportError(frc::err::Error, "{}",
          "Failed to start/restart auto SPI");
      return false;
    }
  }
...
int ADIS16470_IMU::ConfigCalTime(ADIS16470CalibrationTime new_cal_time) { 
  ...
  if(!SwitchToStandardSPI()) {
    FRC_ReportError(frc::err::Error, "{}",
        "Failed to configure/reconfigure standard SPI.");
    return 2;
  }
  ...
  if(!SwitchToAutoSPI()) {
    FRC_ReportError(frc::err::Error, "{}",
        "Failed to configure/reconfigure auto SPI.");
    return 2;
  }
  return 0;
}
...
int ADIS16470_IMU::ConfigDecRate(uint16_t reg) { 
  ...
  if(m_reg > 1999) {
    FRC_ReportError(frc::err::Error, "{}",
        "Attempted to write an invalid decimation value.");
    m_reg = 1999;
  }
...
ADIS16470_IMU::ADIS16470_IMU(ADIS16470_IMU&& rhs) {
  std::swap(m_thread_active, rhs.m_thread_active);
  std::swap(m_first_run, rhs.m_first_run);
  std::swap(m_thread_idle, rhs.m_thread_idle);
  std::swap(m_auto_configured, rhs.m_auto_configured);
  std::swap(m_spi_port, rhs.m_spi_port);
  std::swap(m_calibration_time, rhs.m_calibration_time);
  std::swap(m_spi, rhs.m_spi);
  std::swap(m_auto_interrupt, rhs.m_auto_interrupt);
  std::swap(m_scaled_sample_rate, rhs.m_scaled_sample_rate);
  std::swap(m_acquire_task, rhs.m_acquire_task);
  std::swap(m_sim_device, rhs.m_sim_device);
