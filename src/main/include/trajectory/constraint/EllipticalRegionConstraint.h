...
public:
     ...
     *
     * @param center The center of the ellipse in which to enforce the
     * constraint.
     * @param xWidth The width of the ellipse in which to enforce the
     * constraint.
     * @param yWidth The height of the ellipse in which to enforce the
     * constraint.
     ...
     * region.
     */
    EllipticalRegionConstraint(const Translation2d& center,
                               units::meter_t xWidth, units::meter_t yWidth,
    ...
    EllipticalRegionConstraint(const EllipticalRegionConstraint& rhs)
        : TrajectoryConstraint(rhs) {
        m_center = rhs.m_center;
        m_radii = rhs.m_radii;
        m_constraint = std::make_unique<Constraint>(*(rhs.m_constraint));
    }
    EllipticalRegionConstraint& operator=(
        const EllipticalRegionConstraint& rhs) {
        m_center = rhs.m_center;
        m_radii = rhs.m_radii;
    ...
    EllipticalRegionConstraint(EllipticalRegionConstraint&&) = default;
    EllipticalRegionConstraint& operator=(EllipticalRegionConstraint&&) =
        default;
    ...
    bool IsPoseInRegion(const Pose2d& pose) const {
        ...
        // otherwise it is outside the ellipse. Both sides have been multiplied
        // by Rx^2 * Ry^2 for efficiency reasons.
